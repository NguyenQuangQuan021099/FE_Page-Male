...
let sizeAPI = "http://localhost:3000/listSize";

function start() {
    getItem(renderItems);
    getSize(renderListSize); 
}
start();
...
function renderItems(items){
    ...
    let htmls = items.map(function(item){
        return `
        <li class="product-items col m-6 c-6 l-3">  
...
function renderListSize(listSize){
    let listSizeBlock = document.querySelector(".items-size");
    console.log(listSizeBlock);
    let htmls = listSize.map(function(size){
        return `
            <li class="item"> <a href=""> ${size.sizeClothing}</a></li>
        `;
    });
    listSizeBlock.innerHTML = htmls.join('');
    console.log(htmls);
}
